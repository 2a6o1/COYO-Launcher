...
const launcher = window?.launcher ?? {
  ...
  minecraftGetNews: () =>
    Promise.resolve({
      items: [],
      sourceUrl: "https://www.minecraft.net/en-us/articles",
    }),
  ...
  javaInstall25: () => Promise.resolve(),
  prepareJavaRuntime: () => Promise.resolve({ ok: false, error: 'Not available' }),
...
const ICONS = {
  ...
  console:
    "M4 5h16a1 1 0 011 1v12a1 1 0 01-1 1H4a1 1 0 01-1-1V6a1 1 0 011-1zm0 2v10h16V7H4zm3 7.5l3-2.5-3-2.5v5zm6-1.5h4v2h-4v-2z",
  ...
  return (
    <article className="news-card">
      ...
      <div className="news-card-body">
        ...
        {cardSummary ? (
          <div className="news-card-summary">{cardSummary}</div>
        ) : null}
        <div className="news-card-actions">
          <button
            className="btn-ghost news-open-btn"
            type="button"
    ...
    >
      <div className="modal">
        <div className="modal-title">
          {mode === "edit" ? t("profile.editProfile") : t("profile.newProfile")}
        </div>
        ...
        <div className="modal-field">
          ...
          <div className="modal-field-desc">
            {t("profile.optional")} {t("profile.mainLauncherDecision")}
          </div>
        </div>

        <div className="modal-field">
          <label className="modal-label">
            {t("profile.allocatedRAM", { ram })}
          </label>
          <input
            type="range"
        ...
        <div className="modal-field">
          ...
          />
          <div className="modal-field-desc">
            {t("profile.jvmArgumentsDescription")}
          </div>
        </div>
...
function InstallModal({ type, installTargets, onClose, onInstall }) {
  ...
  const [gameVersion, setGameVersion] = useState(
    usesGameAndLoader ? info.gameVersions?.[0] || "" : info.versions[0] || "",
  );
  const [loaderVersion, setLoaderVersion] = useState(
    usesGameAndLoader
      ? (info.loadersByGameVersion?.[info.gameVersions?.[0] || ""] || [])[0] ||
          ""
      : "",
  );
  ...
  useEffect(() => {
    if (usesGameAndLoader) {
      ...
      const nextLoaderVersion =
        (info.loadersByGameVersion?.[nextGameVersion] || [])[0] || "";
  ...
  const loaderOptions = usesGameAndLoader
    ? info.loadersByGameVersion?.[gameVersion] || []
    : [];
  const canInstall = usesGameAndLoader
    ? Boolean(gameVersion && loaderVersion)
    : Boolean(gameVersion);
    ...
    >
      <div className="modal">
        ...
        <div className="modal-subtitle">
          {usesGameAndLoader
            ? type === "forge"
              ? t("install.selectGameAndForgeRelease")
              : t("install.selectLoaderAndVersion")
        ...
        {usesGameAndLoader ? (
          <>
            <div className="modal-field">
              ...
              <div className="modal-select-meta">
                {t("install.availableVersions", {
                  count: info.gameVersions?.length || 0,
                  plural: (info.gameVersions?.length || 0) === 1 ? "" : "s",
                })}
              </div>
            </div>
            ...
            <div className="modal-field">
              ...
              <div className="modal-select-meta">
                {t("install.availableLoaders", {
                  count: loaderOptions.length,
                  plural: loaderOptions.length === 1 ? "" : "s",
                })}
              </div>
            </div>
          </>
          ...
          <div className="modal-field">
            ...
            <div className="modal-select-meta">
              {t("install.availableVersions", {
                count: info.versions.length,
                plural: info.versions.length === 1 ? "" : "s",
              })}
            </div>
          </div>
        )}
  ...
  return (
    ...
    >
      <div className="modal">
        ...
        <div className="settings-section">
          ...
          <div className="settings-row">
            <div>
              ...
              <div className="settings-row-desc">
                {t("settings.showSnapshotsDesc")}
              </div>
            </div>
            <Toggle
              on={settings.showSnapshots}
              onToggle={() =>
                onChange("showSnapshots", !settings.showSnapshots)
              }
            />
          </div>
          <div className="settings-row">
            <div>
              <div className="settings-row-label">
                {t("settings.keepLauncherOpen")}
              </div>
              <div className="settings-row-desc">
                {t("settings.keepLauncherOpenDesc")}
              </div>
            </div>
            <Toggle
              on={settings.keepOpen}
              onToggle={() => onChange("keepOpen", !settings.keepOpen)}
            />
          </div>
          <div className="settings-row">
            <div>
              <div className="settings-row-label">
                {t("settings.showConsoleOutput")}
              </div>
              <div className="settings-row-desc">
                {t("settings.showConsoleOutputDesc")}
              </div>
            </div>
            <Toggle
              on={settings.showConsole}
              onToggle={() => onChange("showConsole", !settings.showConsole)}
            />
          </div>
          <div className="settings-row">
            <div>
              <div className="settings-row-label">
                {t("settings.autoUpdateLauncher")}
              </div>
              <div className="settings-row-desc">
                {t("settings.autoUpdateLauncherDesc")}
              </div>
            </div>
            <Toggle
              on={settings.autoUpdate}
              onToggle={() => onChange("autoUpdate", !settings.autoUpdate)}
            />
          </div>
          <div className="settings-row">
            <div>
              <div className="settings-row-label">
                {t("settings.minecraftDirectory")}
              </div>
              <div className="settings-row-desc">
                {t("settings.minecraftDirectoryDesc")}
              </div>
            </div>
            <div
              className="settings-directory-controls"
              style={{ maxWidth: "60%" }}
            >
              <input
                className="modal-input"
              ...
              />
              <button
                className="btn-ghost settings-open-folder-btn"
                type="button"
        ...
        <div className="modal-actions">
          <button className="btn-ghost" type="button" onClick={onOpenAbout}>
            {t("settings.aboutButton")}
          </button>
          <button className="btn-ghost" type="button" onClick={onCheckUpdates}>
      ...
      <div className="modal about-modal">
        ...
        <div className="about-hero">
          ...
          <div className="about-version-card">
            <span className="about-version-label">
              {t("settings.versionLabel")}
            </span>
            <span className="about-version-value">{versionLabel}</span>
            <span className="about-version-meta">
        ...
        <div className="about-pills">
          <span className="about-pill">
            {t("settings.licenseLabel", { license: "GPL-2.0" })}
          </span>
          <span className="about-pill">
            {t("settings.developedByLabel", { developer: "CesarGarza55" })}
          </span>
        ...
        <div className="about-grid">
          <div className="about-card">
            <div className="about-card-title">
              {t("about.cards.launcherTitle")}
            </div>
            <div className="about-card-text">
              {t("about.cards.launcherText")}
            </div>
          </div>
          <div className="about-card">
            <div className="about-card-title">
              {t("about.cards.profilesTitle")}
            </div>
            <div className="about-card-text">
              {t("about.cards.profilesText")}
            </div>
          </div>
          <div className="about-card">
...
function BeginnerTutorial({ onClose, onDontShowAgain }) {
  const { t } = useI18n();
  return (
    <>
      <div className="tutorial-backdrop" onClick={onClose} />
      <div className="beginner-tutorial" role="dialog" aria-modal="true">
        <button
          type="button"
          className="tutorial-close"
          onClick={onClose}
          aria-label={t("beginner.tutorial.close")}
          title={t("beginner.tutorial.close")}
        >
          ×
        </button>
        <h2 className="tutorial-title">{t("beginner.tutorial.title")}</h2>
        <p className="tutorial-title-desc">
          {t("beginner.tutorial.titleDesc")}
        </p>
        <ul className="tutorial-steps">
          <li className="tutorial-step">
            <span className="tutorial-step-title">
              {t("beginner.tutorial.step1Title")}
            </span>
            <span className="tutorial-step-desc">
              {t("beginner.tutorial.step1Desc")}
            </span>
          </li>
          <li className="tutorial-step">
            <span className="tutorial-step-title">
              {t("beginner.tutorial.step2Title")}
            </span>
            <span className="tutorial-step-desc">
              {t("beginner.tutorial.step2Desc")}
            </span>
          </li>
          <li className="tutorial-step">
            <span className="tutorial-step-title">
              {t("beginner.tutorial.step3Title")}
            </span>
            <span className="tutorial-step-desc">
              {t("beginner.tutorial.step3Desc")}
            </span>
          </li>
          <li className="tutorial-step">
            <span className="tutorial-step-title">
              {t("beginner.tutorial.step4Title")}
            </span>
            <span className="tutorial-step-desc">
              {t("beginner.tutorial.step4Desc")}
            </span>
          </li>
          <li className="tutorial-step">
            <span className="tutorial-step-title">
              {t("beginner.tutorial.step5Title")}
            </span>
            <span className="tutorial-step-desc">
              {t("beginner.tutorial.step5Desc")}{" "}
              <strong className="tutorial-server-name">
                {t("beginner.tutorial.serverName")}
              </strong>{" "}
              <code className="tutorial-server-address">
                {t("beginner.tutorial.serverAddress")}
              </code>
            </span>
          </li>
        </ul>
        <div className="tutorial-footer">
          <label className="tutorial-checkbox-row">
            ...
            />
            <span>{t("beginner.tutorial.dontShowAgain")}</span>
          </label>
          <button type="button" className="btn-ghost" onClick={onClose}>
            {t("beginner.tutorial.close")}
          </button>
        </div>
      </div>
    </>
...
export default function App() {
  ...
  const [beginnerIsInstalling, setBeginnerIsInstalling] = useState(false);
  ...
  const profileToEdit =
    profileEditor?.profileId != null
      ? profiles.find((p) => p.id === profileEditor.profileId) || null
      : null;
  const accountAvatarUrl = account.loggedIn
    ? getMcHeadsAvatarUrl(account.name, 256)
    : "";
  const activeInstalledVersion =
    installedVersions.find(
      (version) => version.id === activeProfile?.version,
    ) || null;
  const selectedInstalledVersionId = installedVersions.some(
    (version) => version.id === activeProfile?.version,
  )
    ? activeProfile?.version
    : installedVersions[0]?.id || "";
  const selectedInstalledVersion =
    installedVersions.find(
      (version) => version.id === selectedInstalledVersionId,
    ) || null;
  const activeVersion =
    selectedInstalledVersion ||
    activeInstalledVersion ||
    getVersionById(activeProfile?.version, versionCatalog);
  ...
  useEffect(() => {
    let cancelled = false;
    // IPC event listeners
    const installProgressOff = launcher.on?.(
      "minecraft:install-progress",
      (p) => {
        if (!p) return;
        if (p.installId) setInstallId(p.installId);
        if (p && typeof p.percent === "number") setProgress(p.percent);
        if (p && p.percent == null && typeof p.loaded === "number")
          addLog("info", `Installing: ${p.loaded}/${p.total || "unknown"}`);
      },
    );
    const installFileOff = launcher.on?.(
      "minecraft:install-file-progress",
      (f) => {
        if (!f) return;
    ...
    const installCompleteOff = launcher.on?.(
      ...
      (r) => {
        ...
        pushToast({
          ...
          message: t("messages.installSuccessToast", {
            type: t(`install.${r.type}`),
            version: r.version,
          }),
        });
      },
    ...
    const installCancelledOff = launcher.on?.(
      "minecraft:install-cancelled",
      (e) => {
    ...
    const updateProgressOff = launcher.on?.(
      "minecraft:update-progress",
      (u) => {
        if (!u) return;
        setUpdateState((prev) => ({
          ...prev,
          ...
          message: u.assetName
            ? `Downloading ${u.assetName}`
            : prev.message || "Downloading update",
        }));
      },
    );
    const updateCompleteOff = launcher.on?.(
      "minecraft:update-complete",
      (u) => {
        if (!u) return;
        setUpdateState({
          phase: "complete",
    ...
    const javaInstallStatusOff = launcher.on?.("minecraft:java-install-status", (e) => {
      if (!e) return;
      addLog("info", e.message || "Java installation status update");
    });
    const javaInstallProgressOff = launcher.on?.("minecraft:java-install-progress", (e) => {
      if (!e) return;
      if (typeof e.percent === "number") {
        setJavaInstallProgress(e.percent);
      }
    });
    const javaInstallCompleteOff = launcher.on?.("minecraft:java-install-complete", (e) => {
      if (!e) return;
      if (e.success) {
        addLog("success", "Java 25 installed successfully!");
        setJavaInstallProgress(100);
        setTimeout(() => setJavaInstallProgress(0), 2000);
      } else {
        addLog("error", `Java installation failed: ${e.error || "Unknown error"}`);
      }
    });
    ...
    const hydrateCatalog = async () => {
      ...
      const snapshot = await launcher.minecraftGetCatalog({
        includeSnapshots: currentState?.settings?.showSnapshots === true,
      });
    ...
    return () => {
      ...
      try {
        ...
        updateProgressOff?.();
        updateCompleteOff?.();
        javaInstallStatusOff?.();
        javaInstallProgressOff?.();
        javaInstallCompleteOff?.();
        runLogOff?.();
        runExitOff?.();
      } catch (e) {}
    };
  ...
  useEffect(() => {
    ...
    const hydrateState = async () => {
      ...
      const incomingProfiles = Array.isArray(state.profiles)
        ? state.profiles
        : [];
      ...
      const nextProfiles =
        incomingProfiles.length > 0 ? incomingProfiles : [defaultProfile];
      ...
      if (state.minecraftRoot) {
        setSettings((prev) => ({
          ...prev,
          minecraftRoot: state.minecraftRoot,
        }));
      }
      if (state.settings && typeof state.settings === "object") {
        setSettings((prev) => ({
          ...prev,
          keepOpen:
            typeof state.settings.keepOpen === "boolean"
              ? state.settings.keepOpen
              : prev.keepOpen,
          showConsole:
            typeof state.settings.showConsole === "boolean"
              ? state.settings.showConsole
              : prev.showConsole,
          autoUpdate:
            typeof state.settings.autoUpdate === "boolean"
              ? state.settings.autoUpdate
              : prev.autoUpdate,
          showSnapshots:
            typeof state.settings.showSnapshots === "boolean"
              ? state.settings.showSnapshots
              : prev.showSnapshots,
          javaPath:
            typeof state.settings.javaPath === "string"
          ...
          showBeginnerTutorial:
            typeof state.settings.showBeginnerTutorial === "boolean"
              ? state.settings.showBeginnerTutorial
              : true,
        }));
        // Sync the tutorial visibility from persisted settings
        setShowTutorial(
          typeof state.settings.showBeginnerTutorial === "boolean"
            ? state.settings.showBeginnerTutorial
            : true,
      ...
      const hasInstallTargets =
        state.installTargets &&
        typeof state.installTargets === "object" &&
        Object.values(state.installTargets).some(
          (target) =>
            Array.isArray(target?.versions) && target.versions.length > 0,
        );
      ...
      launcher
        ...
        .then((mods) => {
          ...
          const mergedMods = mods.map((mod) => ({
            ...mod,
            ...(savedById.get(mod.id) || {}),
          }));
      ...
      if (incomingProfiles.length === 0) {
        ...
        persistState({
          ...
          logs: [],
          versions:
            state.versions && Array.isArray(state.versions)
              ? state.versions
              : [],
          installTargets:
            state.installTargets && typeof state.installTargets === "object"
              ? state.installTargets
              : installTargets,
          settings:
  ...
  useEffect(() => {
    ...
    const localName = String(
      activeProfile?.localName || activeProfile?.name || "",
    ).trim();
  ...
  const handlePlay = async () => {
    ...
    if (!account.loggedIn || !account.name) {
      addLog(
        "warn",
        "Create a local account name or link Microsoft before launching.",
      );
      return;
    }
    ...
    addLog(
      "info",
      `Launching "${activeProfile.name}" — ${activeVersion.label}`,
    );
  ...
  const handleInstall = async (type, selection) => {
    const usesGameAndLoader = type === "fabric" || type === "forge";
    const payload =
      usesGameAndLoader && selection && typeof selection === "object"
        ? {
    ...
    try {
      ...
      if (res?.error) {
        ...
        pushToast({
          ...
          message: t("messages.installFailedToast", {
            type: t(`install.${type}`),
            version: logLabel,
    ...
    setSettings((prev) => {
      ...
      persistState({
        ...
        settings: {
          ...
          [key]: val,
          javaPath: nextSettings.javaPath,
          language: currentState.settings.language ?? language,
        },
      });
      if (key === "showSnapshots") {
        launcher
          .minecraftGetCatalog?.({ includeSnapshots: Boolean(val) })
          .then((catalog) => {
            if (!catalog) return;
            if (Array.isArray(catalog.versions))
              setVersionCatalog(catalog.versions);
            if (catalog.installTargets)
              setInstallTargets(catalog.installTargets);
          })
          .catch(() => {});
      }
      return nextSettings;
      ...
      setProfiles((prev) => {
        ...
        if (activeProfileId === profileId) {
          setAccount({
            name: "",
            loggedIn: false,
            profileKey: String(nextActiveId ?? "default"),
            kind: "none",
            session: null,
          });
        }
  ...
  const handleDuplicateProfile = (profileId, e) => {
    ...
    addLog(
      "info",
      `Profile "${duplicatedProfile.name}" duplicated from "${sourceProfile.name}".`,
    );
  };
  ...
  const handleModToggle = async (id) => {
    ...
    setMods((prev) =>
      prev.map((mod) =>
        mod.id === id ? { ...mod, enabled: nextEnabled } : mod,
      ),
    );
    ...
    try {
      ...
      setMods((prev) =>
        prev.map((mod) =>
          mod.id === id ? { ...mod, enabled: target.enabled } : mod,
        ),
      );
  ...
  const handleSetAllModsEnabled = async (enable) => {
    ...
    const targetMods = mods.filter(
      (mod) => Boolean(mod.enabled) !== Boolean(enable),
    );
    if (!targetMods.length) return;

    setMods((prev) =>
      prev.map((mod) =>
        Boolean(mod.enabled) !== Boolean(enable)
          ? { ...mod, enabled: enable }
          : mod,
      ),
    );
    ...
    try {
      ...
      addLog(
        "info",
        enable ? t("messages.modsEnabledAll") : t("messages.modsDisabledAll"),
      );
    } catch (error) {
  ...
  const importModFiles = useCallback(
    async (files) => {
      const acceptedFiles = Array.from(files || []).filter((file) =>
        /\.(jar|olpkg)$/i.test(file?.path || file?.name || ""),
      );
      ...
      for (const file of acceptedFiles) {
        ...
        if (!sourcePath) {
          addLog(
            "error",
            `No file path available for ${file?.name || "mod file"}.`,
          );
          continue;
        }
  ...
  const handleAddModClick = useCallback(() => {
    launcher
      ...
      .catch((error) => {
        addLog(
          "error",
          `Could not open mod file picker: ${error?.message || error}`,
        );
      });
  ...
  const handleLogin = async () => {
    ...
    try {
      const session = await minecraftLoginWithAbort(
        profileKey,
        controller.signal,
      );
      if (session?.error) {
      ...
      addLog(
        "success",
        `Authenticated as ${session?.name || "Microsoft account"}.`,
      );
      pushToast({
        tone: "success",
        title: t("account.loginWithMicrosoft"),
        message: t("messages.loginSuccess", {
          name: session?.name || "Microsoft account",
        }),
      });
    } catch (error) {
  ...
  const handleCheckForUpdates = async () => {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("update.launcherUpdate"),
        message: t("messages.updateCheckFailed", {
  ...
  const handleOpenMinecraftDirectory = async () => {
    try {
      ...
      if (result?.error) {
        addLog(
          "error",
          t("messages.openMinecraftDirectoryFailed", {
      ...
      if (result?.ok) {
        addLog(
          "info",
          t("messages.openMinecraftDirectorySuccess", {
            path: result.path || settings.minecraftRoot,
          }),
        );
      }
    } catch (error) {
  ...
  // ── Beginner Mode ───────────────────────────────────────────────────────────
  const handleBeginnerPlay = useCallback(async () => {
    const name = beginnerLocalName.trim();
    if (!name || !beginnerSelectedVersion) return;

    // Detect Java before launching (includes portable runtime check)
    const javaDetection = await launcher.minecraftDetectJava?.();

    if (javaDetection?.error || !javaDetection?.path) {
      // Attempt portable Java runtime download (all platforms, no admin needed)
      setJavaInstalling(true);
      setJavaInstallProgress(0);
      addLog("info", "Java not found. Preparing portable Java runtime...");

      const prepResult = await launcher.prepareJavaRuntime?.();

      setJavaInstalling(false);
      setJavaInstallProgress(0);

      if (prepResult?.error) {
        addLog("error", `Portable Java download failed: ${prepResult.message}`);

        // Fallback: show manual install prompt with Adoptium link
        const javaLink = process.platform === 'win32'
          ? "https://api.adoptium.net/v3/binary/latest/26/ga/windows/x64/jdk/hotspot/normal/eclipse"
          : "https://adoptium.net";
        pushToast({
          tone: "error",
          title: "Java Not Found",
          message: "Could not download a portable Java runtime automatically. Please install Java 21+ manually.",
          duration: 0,
          actions: [{
            label: "Download Java 26",
            onClick: () => window.open(javaLink)
          }]
        });
        return;
      }

      // Re-detect Java after portable runtime is ready
      addLog("info", "Re-detecting Java after portable runtime download...");
      const recheck = await launcher.minecraftDetectJava?.();

      if (recheck?.error || !recheck?.path) {
        addLog("error", "Java still not found after portable runtime preparation");
        pushToast({
          tone: "error",
          title: "Java Not Found",
          message: "Java runtime was downloaded but could not be detected. Please restart the launcher.",
        });
        return;
      }

      addLog("success", `Java detected at ${recheck.path} after portable runtime installation`);
    }

    addLog("info", `Launching Minecraft with Java at ${javaDetection?.path || "(auto-detected)"}`);
    ...
    handleSaveProfile({
      ...
      ram: 4,
      jvmArguments: "",
      javaPath: javaDetection?.path || settings.javaPath || "", // ← CRUCIAL: pasa la ruta de Java
    });
    setAccount({
      name,
      loggedIn: true,
  ...
  }, [beginnerLocalName, beginnerSelectedVersion, addLog, pushToast, handleSaveProfile, setAccount, setBeginnerPlayIntent, addLog]);

  const handleBeginnerSelectAndPlay = (profile) => {
    if (!profile) return;
    setActiveProfileId(profile.id);
    // Sincronizar estado del formulario con el perfil seleccionado
    setBeginnerLocalName(profile.localName || profile.name || "");
    setBeginnerSelectedVersion(profile.version || "");
    // Establecer cuenta offline con el nombre del perfil
    setAccount({
      name: profile.localName || profile.name || "",
      loggedIn: true,
      profileKey: "default",
      kind: "local",
      session: createOfflineSession(profile.localName || profile.name || ""),
    });
    setBeginnerPlayIntent(true);
  };

  const handleBeginnerInstall = useCallback(
    async (mcVer) => {
      if (!mcVer) return;
      setBeginnerIsInstalling(true);
      setProgress(0);
      try {
        const res = await install({ type: "minecraft", version: mcVer });
        if (res?.error) {
          addLog("error", res.message || res.error);
        } else {
          addLog("success", `Minecraft ${mcVer} installation requested.`);
        }
      } catch (err) {
        addLog("error", `Install failed: ${err?.message || err}`);
      } finally {
  ...
  // Dismiss the beginner tutorial overlay; optionally persist "don't show again"
  const dismissTutorial = useCallback(
    (dontShowAgain = false) => {
      setShowTutorial(false);
      const nextSetting = !dontShowAgain;
      setSettings((prev) => ({ ...prev, showBeginnerTutorial: nextSetting }));
      const currentState = stateRef.current;
      persistState({
        profiles: currentState.profiles,
        mods: currentState.mods,
        ...
        settings: {
          ...currentState.settings,
          showBeginnerTutorial: nextSetting,
        },
      });
    },
    [installTargets, persistState, versionCatalog],
    ...
    <>
      ...
      <div className="titlebar">
        ...
        <div className="titlebar-controls">
          <button
            className="win-btn"
            onClick={() => launcher.windowMinimize()}
          ...
          <button
            className="win-btn"
            onClick={() => launcher.windowMaximize()}
      ...
      {beginnerMode ? (
        <div className="beginner-mode">
          <CentralCard
            ...
            onInstallVersion={handleBeginnerInstall}
            isInstalling={beginnerIsInstalling}
            ...
            profiles={profiles}
            onSelectProfile={handleBeginnerSelectAndPlay}
          />
          {showTutorial ? (
            <BeginnerTutorial
              onClose={() => dismissTutorial(false)}
              onDontShowAgain={() => dismissTutorial(true)}
            />
          ) : null}
        </div>
      ) : (
        <div className="app-layout">
          ...
          <aside className="sidebar">
            <div className="sidebar-section">
              ...
              {profiles.length === 0 ? (
                ...
                profiles.map((p) => {
                  const installedProfileVersion = p.version
                    ? installedVersions.find(
                        (version) => version.id === p.version,
                      ) || getVersionById(p.version, versionCatalog)
                    : null;
                  const profileVersionLabel =
                    installedProfileVersion?.mcVer || "No version";
                  const profileVersionType =
                    installedProfileVersion?.type ||
                    t("profile.launcherSelector");
                  ...
                  return (
                    <div
                      ...
                      style={{
                        opacity:
                          authRefreshing || gameState === "running" ? 0.6 : 1,
                        cursor:
                          authRefreshing || gameState === "running"
                            ? "not-allowed"
                            : "pointer",
                      }}
                    >
                      <div className="profile-card-header">
                        ...
                        <ProfileCardActions
                          ...
                          onEdit={(e) => {
                            if (authRefreshing || gameState === "running")
                              return;
                            e.stopPropagation();
                      ...
                      >
                        {p.version ? (
                          <span className="profile-version-chip profile-version-primary">
                            {profileVersionLabel}
                          </span>
                        ) : (
                          <span className="profile-version-chip">
                            {t("profile.useLauncherSelector")}
                          </span>
                        )}
                        <span className="profile-version-chip">
                          {p.ram || 4}GB RAM
                        ...
                        {p.javaPath && (
                          <span
                            className="profile-version-chip"
                            title={p.javaPath}
                          >
                            JVM
                          </span>
              ...
              <button
                ...
                onClick={() =>
                  setProfileEditor({ mode: "new", profileId: null })
                }
                disabled={authRefreshing || gameState === "running"}
              >
                ...
                <div className="account-card">
                  <div className="account-top">
                    ...
                    <div className="account-info">
                      ...
                      <div
                        className={`account-status ${gameState === "running" ? "online" : "offline"}`}
                      >
                        {authRefreshing ? (
                          <>
                            <span
                            ...
                            >
                              <span className="play-btn-icon spinning">
                  ...
                  {!authRefreshing && account.kind === "microsoft" ? (
                    <button
                      className="ms-link-btn account-action-btn"
          ...
          <main className="main-content">
              ...
              <div className={`update-banner ${updateState.phase}`}>
                <div className="update-banner-copy">
                  <div className="update-banner-title">
                    {t("update.launcherUpdate")}
                  </div>
                  <div className="update-banner-text">
                    {updateState.message || t("update.preparingUpdate")}
                  </div>
                </div>
            ...
            {/* Console tab */}
            <div
              className={`tab-panel ${activeTab === "console" ? "active" : ""}`}
              ...
              >
                <div className="console-header">
                  <div className="console-title">
                    <div
                      className={`console-indicator ${gameState === "running" ? "running" : ""}`}
                    />
                    {t("console.outputLog")}
                    {gameState === "loading" && (
                      <span
                  ...
                  {settings.showConsole ? (
                    ...
                    >
                      ...
                      <button
                        ...
                        title={
                          autoScroll
                            ? t("console.disableScroll") || "Pausar Scroll"
                            : t("console.enableScroll") || "Reanudar Scroll"
                        }
                      ...
                      >
                        <Icon
                          d={autoScroll ? ICONS.play : ICONS.stop}
                          size={11}
                        />
                      </button>
                      <button
                  ...
                  >
                    {(() => {
                      const filteredLogs = logs.filter((entry) => {
                        ...
                        const entryLevel = String(
                          entry.level || "",
                        ).toLowerCase();
                        if (logFilter === "warn")
                          return (
                            entryLevel === "warn" ||
                            String(entry.msg || "").includes("WARN")
                          );
                        if (logFilter === "error")
                          return (
                            entryLevel === "error" ||
                            String(entry.msg || "").includes("ERROR") ||
                            String(entry.msg || "").includes("Exception") ||
                            String(entry.msg || "").includes("FATAL")
                          );
                        if (logFilter === "info")
                          return (
                            entryLevel === "info" ||
                      ...
                      return filteredLogs.map((entry, i) => (
            ...
            >
              <div className="news-header">
                ...
                <div className="news-actions">
                  <button
                    className="btn-ghost news-refresh-btn"
                    type="button"
                    onClick={loadNews}
                  ...
                  <button
                    className="btn-primary news-source-btn"
                    type="button"
                    onClick={handleOpenNewsSource}
                  >
                    <Icon d={ICONS.external} size={11} />
                    {t("news.openSource")}
                  </button>
            ...
            {/* Mods tab */}
            <div
              className={`tab-panel ${activeTab === "mods" ? "active" : ""}`}
              style={{
            ...
            >
              ...
              >
                ...
                >
                  {mods.filter((m) => m.enabled).length} {t("mods.active")} ·{" "}
                  {mods.filter((m) => !m.enabled).length} {t("mods.disabled")}
                </span>
                <input
                  type="text"
                  className="modal-input"
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    ...
                    onClick={() => handleSetAllModsEnabled(true)}
                    disabled={!mods.some((mod) => !mod.enabled)}
                  >
                  ...
                  <button
                    className="action-btn"
                    style={{ fontSize: 11, padding: "5px 10px" }}
                    onClick={() => handleSetAllModsEnabled(false)}
                    disabled={!mods.some((mod) => mod.enabled)}
                  >
              ...
              <div className="ram-indicator">
                <div className="ram-bar">
                  <div
                    className="ram-fill"
                    style={{
                      width: `${Math.min(100, ((activeProfile?.ram || 4) / 16) * 100)}%`,
                    }}
                  />
                </div>
                {activeProfile?.ram || 4}
      ...
      {(modal === "install-minecraft" ||
        modal === "install-fabric" ||
        modal === "install-forge") && (
        <InstallModal
