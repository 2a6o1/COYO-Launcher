...
// Java version to bundle as portable runtime (latest LTS from Adoptium).
// Java 26 is the latest feature release; Java 21+ is required for Minecraft 1.21.x+.
const PORTABLE_JAVA_VERSION = 26;
const PORTABLE_JAVA_OS = process.platform === 'win32' ? 'windows' : process.platform === 'darwin' ? 'macos' : 'linux';
const PORTABLE_JAVA_ARCH = process.arch === 'arm64' ? 'aarch64' : 'x64';
const PORTABLE_JAVA_URL = `https://api.adoptium.net/v3/binary/latest/${PORTABLE_JAVA_VERSION}/ga/${PORTABLE_JAVA_OS}/${PORTABLE_JAVA_ARCH}/jdk/hotspot/normal/eclipse`;
const PORTABLE_JAVA_DIR = path.join(app.getPath('userData'), 'java-runtime');
const PORTABLE_JAVA_MARKER = path.join(PORTABLE_JAVA_DIR, '.installed');
...
function getUpdaterStrings(language) {
  return UPDATER_TRANSLATIONS[language] || UPDATER_TRANSLATIONS.es;
}
...
// === Portable Java Runtime: Download + extract portable JDK without admin rights ===
function findPortableJava() {
  try {
    const entries = fs.readdirSync(PORTABLE_JAVA_DIR, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;
      const javaBinName = process.platform === 'win32' ? 'java.exe' : 'java';
      const candidate = path.join(PORTABLE_JAVA_DIR, entry.name, 'bin', javaBinName);
      try {
...
function isPortableJavaInstalled() {
  return fs.existsSync(PORTABLE_JAVA_MARKER) && findPortableJava() !== null;
}

async function prepareJavaRuntime() {
  try {
    log.info('[PortableJava] Preparing portable Java runtime');

    // 1. If already downloaded, locate and return the path
    const existingPath = findPortableJava();
    if (existingPath) {
      log.info('[PortableJava] Reusing existing portable Java at', existingPath);
      return { ok: true, javaPath: existingPath, version: PORTABLE_JAVA_VERSION, reused: true };
    }

    // 2. Download the portable JDK archive (ZIP for Windows, TAR.GZ for Unix)
    sendUpdateEvent('minecraft:java-install-status', {
      phase: 'downloading',
      message: `Downloading Java ${PORTABLE_JAVA_VERSION} (portable)...`,
    });

    const isWindows = process.platform === 'win32';
    const archivePath = path.join(PORTABLE_JAVA_DIR, `java${PORTABLE_JAVA_VERSION}${isWindows ? '.zip' : '.tar.gz'}`);
    ...
    await downloadFileToPath(
      ...
      {
        onProgress: ({ percent }) => {
          sendUpdateEvent('minecraft:java-install-progress', {
            percent: percent ?? 0,
            phase: 'downloading',
          });
        },
    ...
    log.info('[PortableJava] Download complete, extracting...');
    sendUpdateEvent('minecraft:java-install-status', {
      phase: 'extracting',
      message: 'Extracting Java runtime...',
    });

    // 3. Extract the archive into PORTABLE_JAVA_DIR
    await fs.promises.mkdir(PORTABLE_JAVA_DIR, { recursive: true });

    if (isWindows) {
      const zip = new AdmZip(archivePath);
      zip.extractAllTo(PORTABLE_JAVA_DIR, true);
    } else {
      // Unix: extract tar.gz
      const { execSync } = await import('child_process');
      execSync(`tar -xzf "${archivePath}" -C "${PORTABLE_JAVA_DIR}"`, { stdio: 'ignore' });
    }
    ...
    try {
      ...
      log.warn('[PortableJava] Failed to clean up archive:', cleanupError?.message);
    }

    // 5. Locate the java executable
    const javaPath = findPortableJava();
    if (!javaPath) {
      throw new Error('Java executable not found after extraction.');
    }

    // 6. Write marker to indicate successful installation
    await fs.promises.writeFile(
      PORTABLE_JAVA_MARKER,
      JSON.stringify({ version: PORTABLE_JAVA_VERSION, installedAt: new Date().toISOString() }),
      'utf8',
    );

    log.info('[PortableJava] Java runtime ready at', javaPath);
    sendUpdateEvent('minecraft:java-install-complete', { success: true, javaPath });
    return { ok: true, javaPath, version: PORTABLE_JAVA_VERSION, reused: false };
  } catch (error) {
    const errorMsg = error?.message || String(error);
    log.error('[PortableJava] Failed:', errorMsg);
    sendUpdateEvent('minecraft:java-install-complete', { success: false, error: errorMsg });
    return { error: 'PortableJavaFailed', message: errorMsg };
  }
}

// === NUEVA FUNCIÓN: Instala Java 25 automáticamente en Windows sin intervención del usuario ===
// Descarga el instalador MSI de Adoptium y lo instala silenciosamente via msiexec.
async function installJava25() {
  try {
    log.info('[JavaInstaller] Starting Java 25 installation for Windows');

    // Directorio temporal para el instalador MSI
    const javaInstallerDir = path.join(app.getPath('userData'), 'java-installer');
    await fs.promises.mkdir(javaInstallerDir, { recursive: true });
    const installerPath = path.join(javaInstallerDir, 'java25.msi');

    // 1. Descargar instalador MSI de Adoptium (Java 25 GA Windows x64)
    sendUpdateEvent('minecraft:java-install-status', {
      phase: 'downloading',
      message: 'Downloading Java 25...'
    });
    ...
    log.info('[JavaInstaller] Download complete');
    sendUpdateEvent('minecraft:java-install-status', {
      phase: 'installing',
      message: 'Installing Java 25...'
    });

    // 2. Instalar silenciosamente con msiexec
    //    Flags: /quiet (no UI), /norestart (no reiniciar), AUTO_UPDATE=0, REBOOT=0
    const installResult = await new Promise((resolve) => {
      let resolved = false;

      const child = spawn('msiexec', [
        '/i', installerPath,
        '/quiet',
        '/norestart',
        'AUTO_UPDATE=0',
      ...
      child.on('close', (code) => {
        if (!resolved) {
          resolved = true;
          log.info(`[JavaInstaller] msiexec exited with code ${code}`);
      ...
      child.on('error', (error) => {
        if (!resolved) {
          resolved = true;
          log.error('[JavaInstaller] Spawn error:', error?.message);
      ...
      setTimeout(() => {
        if (!resolved) {
          resolved = true;
          log.warn('[JavaInstaller] Installation timed out after 5 minutes');
          resolve({ success: false, error: 'Installation timeout (5 min)' });
        }
      }, 300000);
    });
    ...
    try {
      ...
      log.warn('[JavaInstaller] Failed to clean up installer file:', cleanupError?.message);
    }

    // 4. Reportar resultado
    if (installResult.success || installResult.exitCode === 0) {
      log.info('[JavaInstaller] Java 25 installed successfully');
      sendUpdateEvent('minecraft:java-install-complete', { success: true });
      return { ok: true };
    } else {
      const errorMsg = installResult.error || `msiexec exited with code ${installResult.exitCode}`;
      log.error('[JavaInstaller] Installation failed:', errorMsg);
      sendUpdateEvent('minecraft:java-install-complete', { success: false, error: errorMsg });
      return { error: 'InstallFailed', message: errorMsg };
    }
  } catch (error) {
    log.error('[JavaInstaller] Exception:', error?.message || error);
    sendUpdateEvent('minecraft:java-install-complete', { success: false, error: error?.message || String(error) });
    return { error: 'Exception', message: error?.message || String(error) };
  }
}

async function checkForLauncherUpdate({ promptUser = false, parentWindow = null } = {}) {
  // Updates disabled — launcher no longer self-updates
  const currentVersion = String(app.getVersion() || '').trim();
  sendUpdateEvent('minecraft:update-status', { phase: 'up-to-date', currentVersion, latestVersion: currentVersion, message: 'Updates are disabled for this build.' });
  return { ok: true, upToDate: true, currentVersion, latestVersion: currentVersion, skipped: true };
}
    ...
    for (const root of roots) {
      try {
        ...
        for (const entry of entries) {
          if (!entry.isDirectory()) continue;
          const lower = entry.name.toLowerCase();
          if (!/(jdk|jre|java|temurin|adoptium|corretto|zulu|bellsoft|openlogic|microsoft|openjdk)/.test(lower)) continue;
          add(path.join(root, entry.name, 'bin', 'java'));
        }
      } catch {}
    }
  }

  // Add portable Java runtime from userData directory (highest priority for auto-install)
  const portableJava = findPortableJava();
  if (portableJava) {
    candidates.push(portableJava);
  }
...
// === Expose Java detection as a simple async function for IPC ===
async function detectJava(opts) {
  const preferredPath = opts?.javaPath || '';
  const requiredMajor = Number(opts?.requiredMajor || 0) || 0;
  const choice = findJavaCommand(requiredMajor || 8, preferredPath);
  return {
    javaPath: choice.javaCmd,
    javaMajor: choice.javaMajor,
    path: choice.javaCmd,
...
async function createWindow() {
  ...
  if (isDev) {
    try {
      ...
      log.error('Failed to load Vite dev server at http://localhost:5173:', e?.message || e);
      return;
    }
  }
  ...
  if (!loaded) {
    try {
      ...
      log.error('Failed to load index.html from candidates:', err?.message || err);
    }
  }
...
async function maybeRunStartupUpdateCheck() {
  try {
    ...
    log.warn('[StartupUpdateCheck] Failed:', error?.message || error);
  }
}

app.whenReady().then(async () => { await createWindow(); /* Updates disabled — check with dev team before re-enabling */ });
...
// Detect Java (exposed to renderer via IPC)
ipcMain.handle('minecraft:detect-java', async (_, opts) => {
  return detectJava(opts || {});
});
...
// Updates disabled — this IPC handler returns "up-to-date" so the frontend shows no update dialog
ipcMain.handle('minecraft:check-update', async () => {
  const currentVersion = String(app.getVersion() || '').trim();
  return { ok: true, upToDate: true, currentVersion, latestVersion: currentVersion, skipped: true };
});

// Java 25 auto-installer (Windows only)
ipcMain.handle('minecraft:install-java25', async () => {
  if (process.platform !== 'win32') {
    return { error: 'UnsupportedPlatform', message: 'Java auto-install is only supported on Windows.' };
  }
  return installJava25();
});

// Portable Java runtime: downloads + extracts portable JDK without admin rights (all platforms)
ipcMain.handle('minecraft:prepare-java-runtime', async () => {
  return prepareJavaRuntime();
});
  ...
  try {
    ...
    if (type === 'forge') {
      ...
      if (patchResult.code !== 0) {
        log.info('First install attempt failed, trying alternative method...');
...
ipcMain.handle('minecraft:run', async (_, opts) => {
  ...
  // Priority 3: Java path from the saved launcher settings
    ...
    try {
      ...
      log.warn('Failed to load saved Java path:', error);
    }
  }
  ...
  async function findCompatibleJava() {
    ...
    // Attempt to prepare the portable Java runtime (downloads if not yet installed).
    // Used as a fallback when no compatible system Java is found for the required version.
    async function ensurePortableJavaRuntime() {
      if (isPortableJavaInstalled()) {
        const portablePath = findPortableJava();
        if (portablePath) {
          return { ok: true, javaPath: portablePath, version: PORTABLE_JAVA_VERSION, reused: true };
        }
      }
      const prepResult = await prepareJavaRuntime();
      if (prepResult?.ok) {
        const portablePath = findPortableJava();
        if (portablePath) {
          return { ok: true, javaPath: portablePath, version: PORTABLE_JAVA_VERSION, reused: false };
        }
      }
      return prepResult || { error: 'PortableJavaNotReady', message: 'Portable Java runtime is not ready.' };
    }

    // No compatible system Java found — attempt to prepare the portable Java runtime.
    // This downloads Java 26 (which meets the 25+ requirement) if it hasn't been
    // downloaded yet, avoiding requiring the user to manually install Java.
    if (requiredJavaMajor) {
      try {
        sendUpdateEvent('minecraft:run-log', {
          type: 'info',
          msg: `No compatible Java found for version requirement (needs Java ${requiredJavaMajor}+). Attempting to download portable Java runtime...`,
        });
        const portableResult = await ensurePortableJavaRuntime();
        if (portableResult?.ok && portableResult.javaPath) {
          const portableMajor = detectJavaMajor(portableResult.javaPath);
          if (portableMajor >= requiredJavaMajor) {
            return {
              javaCmd: portableResult.javaPath,
              javaMajor: portableMajor,
              source: portableResult.reused ? 'portable (reused)' : 'portable (downloaded)',
            };
          }
        }
      } catch (portableError) {
        log.warn('[findCompatibleJava] Portable Java preparation failed:', portableError?.message || portableError);
      }
    }
...
ipcMain.handle('app:get-language', async () => {
  try { const state = await loadLauncherState(app.getPath('userData')); return state.settings?.language || 'es'; }
  catch { return 'es'; }
});
...
ipcMain.handle('app:set-language', async (_, language) => {
  try {
    ...
    await saveLauncherState(app.getPath('userData'), {
      ...currentState,
      settings: { ...currentState.settings, language: typeof language === 'string' && language.trim() ? language.trim() : 'es' },
    });
    return { ok: true };
